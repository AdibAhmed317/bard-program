  ...
  return (
    <AdminShell title='Dashboard'>
        ...
        {cards.map(({ icon: Icon, label, value, hint }) => (
          <div key={label} className='card p-6'>
            <Icon
              ...
              strokeWidth={1.7}
            />
            <p className='mt-4 text-3xl font-extrabold text-[var(--charcoal-900)]'>
              {value}
            </p>
            <p className='text-sm font-bold text-[var(--charcoal-700)]'>
              {label}
            </p>
      ...
      <div className='card mt-6 p-6'>
        <p className='text-sm font-bold text-[var(--charcoal-900)]'>
        ...
        <ul className='mt-4 divide-y divide-[var(--hairline)]'>
          {participants.slice(0, 6).map((p) => (
            <li key={p.id} className='flex items-center justify-between py-3'>
              <div>
                <p className='font-semibold text-[var(--charcoal-900)]'>
                  {p.name}
                </p>
              ...
              <span
                className={`pill ${p.roomNumber ? 'pill-present' : 'pill-neutral'}`}
              >
          ...
          {participants.length === 0 && (
            <li className='py-3 text-sm text-[var(--charcoal-500)]'>
